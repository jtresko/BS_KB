---
title: "An Unexpected $SPY Reversal"
layout: default
parent: Trading Walkthroughs
nav_order: 3
grand_parent: Strategy
---

This is an example of how BigShort can help traders. Data from SPY on the [SF Segregated Tab](/getting-started/platform-tabs/SF Segregated Tab/) on February 3rd, 2025, 5 min chart.   
  
[Click here for a direct link to the Google Slides](https://docs.google.com/presentation/d/11Yo_azCYPh5bhdP7tl6FH1EENdpsTHYSwqymGJx5lmw/edit?usp=sharing) version of this presentation.  


![](/assets/images/help/strategy/An Unexpected $SPY R_TW+-+Reversal+-+1.png)

![](/assets/images/help/strategy/An Unexpected $SPY R_TW+-+Reversal+-+2.png)

  


![](/assets/images/help/strategy/An Unexpected $SPY R_TW+-+Reversal+-+3.png)

  


![](/assets/images/help/strategy/An Unexpected $SPY R_TW+-+Reversal+-+4.png)

  


![](/assets/images/help/strategy/An Unexpected $SPY R_TW+-+Reversal+-+5.png)

  


![](/assets/images/help/strategy/An Unexpected $SPY R_TW+-+Reversal+-+6.png)

  


![](/assets/images/help/strategy/An Unexpected $SPY R_TW+-+Reversal+-+7.png)

A note to up your game:

  * SmartFlow can be manipulative and manipulated. Be careful to trade confluences and understand the role of each indicator.


  
For more info on the indicators and patterns used in this play-by-play:

  * To learn more about the [SF Segregated](/getting-started/platform-tabs/SF Segregated Tab/) tab

  * To learn more about [Net Options Flow (NOF)](/indicators-and-features/Net Option Flow A Complete Guide/)

  * To learn more about [Momo and SmartFlow](/indicators-and-features/Understanding SmartFlow and MomoFlow Indicators/)

  * To learn more about [Dark Pools (DPs)](/indicators-and-features/Understanding Dark Pools and DarkFlow in BigShort/)  
  



* * *

  
** _Disclaimer:_**  
 _The information provided in this article is for educational and informational purposes only and should not be considered as financial advice. BigShort does not provide personalized investment recommendations or advice, and you should not make any trading or investment decisions based solely on the content of this article. Trading in the stock market involves significant risk, including the potential loss of your entire investment. Past performance of our tools, indicators, or any referenced strategies does not guarantee future results. Always do your own research and consult with a qualified financial advisor before making investment decisions. Use of our platform and its features is at your own risk._

  
